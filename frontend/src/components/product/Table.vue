    ...
    <el-table :data="tableList" stripe @row-click="openDialog">
      ...
      <el-table-column label="操作">
        <template v-slot="scope">
          <el-button type="danger" :icon="Delete" :disabled="scope.row.name === currentName" @click.stop="remove(scope.row.id)"/>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
...
import { ArrowRight, Search, Delete } from '@element-plus/icons-vue'; // 导入图标
...
const remove = (tableId) => {
  ElMessageBox.confirm(
    ...
    .then(() => {
      // 假设 tableList 是你的用户列表
      const index = tableList.value.findIndex(table => table.id === tableId);
      ...
      if (index !== -1) {
        ...
        const removedTables = JSON.parse(localStorage.getItem('removedUsers') || '[]');
        removedTables.push(tableId);
        localStorage.setItem('removedTables', JSON.stringify(removedTables));
