...
public class ProductController {
    ...
    @PostMapping("/delete-one-product")
    public R deleteOneProduct(@RequestBody Product product){
        int flag = productService.deleteOneProduct(product);
        if (flag != 1){
            return new R(Code.WORK_ERR,"删除产品失败！");
        }else {
            return new R(Code.WORK_OK,"删除产品成功！");
        }
    }
